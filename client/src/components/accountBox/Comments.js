...
function Comments({ projectId }) {
  ...
  const renderComments = () => {
    ...
    return comments.map((comment, i) => (
      <div key={i} className="flex flex-row gap-3 border border-secondary px-4 py-4 rounded-xl mb-4 justify-start">
        <div className="w-8 h-8 rounded-full overflow-hidden border border-primary">
          <Profile />
        </div>
        <div className="flex flex-col justify-start">
          <h4 className="text-white text-sm opacity-50 flex flex-col justify-between">@{comment.commentAuthor}</h4>
          <h3 className="comment">{comment.commentText}</h3>
        </div>
      </div>
    ));
